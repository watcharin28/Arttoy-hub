...
const ProductDetail = () => {
    ...
    useEffect(() => {
        ...
        if (token) {
            try {
                ...
                axios
                    .get(`http://localhost:8080/api/user/favorites/status/${product_id}`, {
                        withCredentials: true,
                    })
                    .then((response) => {
    ...
    const handleAddToCart = async () => {
        ...
        try {
            ...
            alert("Add products to shopping cart");
        } catch (err) {
    ...
    return (
        <div className="min-h-screen">
            ...
            {/* Main Content */}
            <div className="bg-white max-w-7xl mx-auto px-20 py-12">
                        ...
                        <div className="flex gap-3 mt-3 overflow-x-auto">
                            {product.images.map((img, i) => (
                                <img
                                    ...
                                    className={`h-16 w-16 rounded-md cursor-pointer object-cover border-2 ${i === selectedImageIndex ? "border-blue-500" : "border-transparent"
                                        }`}
                                />
                            ))}
                        </div>
                ...
                <div className="mt-6 w-full bg-gray-100 rounded-md p-6 shadow-sm">
                    {/* คำอธิบายสินค้า */}
                    <div className="mb-6">
                        <h3 className="font-bold text-black mb-4 text-xl">Product Description</h3>
                        <p className="text-gray-900 mb-2 text-lg font-medium">{product.description}</p>
                        <div className="mt-2 text-gray-700 space-y-2">
                            <div className="flex items-center">
                                <span className="w-24 text-base text-gray-500">Category</span>
                                <span className="text-lg font-medium  text-black">{product.category}</span>
                            </div>
                            <div className="flex items-center">
                                <span className="w-24 text-base text-gray-500">Type</span>
                                <span className="text-lg font-medium  text-black">{product.type === "checked_with_card" ? "Checked with Card" : "Unseal"}</span>
                            </div>
                            <div className="flex items-center">
                                <span className="w-24 text-base text-gray-500">Color</span>
                                <span className="text-lg font-medium  text-black">{product.color}</span>
                            </div>
                            <div className="flex items-center">
                                <span className="w-24 text-base text-gray-500">Size</span>
                                <span className="text-lg font-medium  text-black">{product.size}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-6 w-full bg-gray-100 rounded-md p-6 shadow-sm">
                    ...
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <button
                            onClick={() => alert(`Go to ${product.seller.name} store`)}
                    ...
                    <div className="flex space-x-4 overflow-x-auto pb-2">
                        {reviews.map((review) => (
                            ...
                            >
                                ...
                                />
                                <span className="font-semibold text-gray-800 mb-1">{review.userName}</span>
                                <div className="mb-2">{renderStars(review.rating)}</div>
                                <p className="text-base text-gray-700 mb-3 px-2">"{review.comment}"</p>
